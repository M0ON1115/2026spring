...
검증 기록은 체크리스트 각 항목 아래에 붙는 메모가 아니라, 독립적인 기록 목록으로 관리됩니다. 사용자는 인터뷰 결과, 설문조사 내용, 경쟁 서비스 조사, 아이디어 수정 방향 등을 기록 단위로 추가하고 필요할 경우 삭제할 수 있습니다.

자동 변경 이력은 체크리스트 변경, 프로젝트 메모 수정, 검증 기록 추가·삭제가 발생할 때 기록됩니다.
...
체크리스트, 프로젝트 메모, 검증 기록, 자동 변경 이력을 통해 아이디어를 실제 검증 과정으로 연결합니다.
...
> **KU STARTUP PLANNER**는 사용자의 전공, 경험, 자본과 시간을 기반으로 실행 가능한 창업 아이템을 추천하고, AI 기본 분석·최신 경제 동향·시장 검증 체크리스트와 검증 기록을 연결하여 하나의 창업 프로젝트로 발전시키는 서비스입니다.

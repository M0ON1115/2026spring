# 2024130871 신희문
* 최신 경제 동향 분석 기능은 시간이 조금 필요합니다. 같은 메세지가 반복해서 출력되더라도 오류가 아니니 잠시만 기다려주세요!
  
# KU STARTUP PLANNER

[https://www.m0on1115.com](https://www.m0on1115.com/)

> **개인의 조건을 반영해 실행 가능한 창업 아이템을 추천하고, AI 기본 분석·최신 경제 동향·시장 검증 체크리스트·메모/요약 기록까지 하나의 프로젝트로 관리하는 창업 기획 워크스페이스**
...
단순한 아이디어 생성에서 끝나지 않고, 선택한 아이템을 하나의 프로젝트로 저장하여 기본 분석, 경제 전망, 시장 검증 체크리스트, 프로젝트 메모와 검증 기록까지 이어서 관리할 수 있습니다.
...
체크리스트는 시장 검증 진행률 계산에 사용됩니다. 세부 인터뷰 내용, 조사 결과, 아이디어 수정 방향은 각 체크리스트 항목 아래에 따로 적는 방식이 아니라, 별도의 **검증 기록**으로 추가해 관리합니다.
...
시장 검증 체크리스트 옆에 별도의 **메모/요약** 탭을 두어, 시장 검증 과정에서 확인한 내용을 체크리스트와 분리해 관리합니다.
...
검증 기록은 체크리스트 각 항목 아래에 붙는 메모가 아니라, 독립적인 기록 목록으로 관리됩니다. 사용자는 인터뷰 결과, 설문조사 내용, 경쟁 서비스 조사, 아이디어 수정 방향 등을 기록 단위로 추가하고, 필요할 경우 수정하거나 삭제할 수 있습니다.

자동 변경 이력은 체크리스트 변경, 프로젝트 메모 수정, 검증 기록 추가·수정·삭제가 발생할 때 기록됩니다.
...
13. 검증 기록 추가·수정·삭제
14. 프로젝트 전체 메모 작성
15. 자동 변경 이력과 분석 기록 요약 확인
16. 프로젝트 저장
17. 이후 다시 로그인하여 프로젝트 이어하기
```
...
시장 검증 체크리스트와 별도의 메모/요약 탭을 통해 검증 기록을 추가·수정·삭제하고, 아이디어를 실제 검증 과정으로 연결합니다.
...
> **KU STARTUP PLANNER**는 사용자의 전공, 경험, 자본과 시간을 기반으로 실행 가능한 창업 아이템을 추천하고, AI 기본 분석·최신 경제 동향·시장 검증 체크리스트·메모/요약 기록을 연결하여 하나의 창업 프로젝트로 발전시키는 서비스입니다.

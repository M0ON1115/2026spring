...
> 가장 먼저 해결해야 하는 우선 과제입니다.

- [ ] Preview 배포와 Production 배포 분리 확인
- [ ] 배포된 `index.html`과 `js/forecast.js` 버전 확인
- [ ] 브라우저 캐시와 정적 파일 캐시 검증
- [ ] 경제 전망 결과 화면 최종 검수
- [ ] 저장 후 재호출 결과 일관성 확인
- [ ] 오류 발생 시 사용자 안내 문구 개선
...
> **KU STARTUP PLANNER**는 사용자의 전공, 경험, 자본과 시간을 기반으로 실행 가능한 창업 아이템을 추천하고, AI 기본 분석과 최신 경제 동향을 연결하여 하나의 창업 프로젝트로 발전시키는 서비스입니다.
